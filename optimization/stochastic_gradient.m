function [wavg, w] = stochastic_gradient(w0, params, obj_grad_func)
...
if (~isfield(params, 'numIters')),
	error('When used with stochastic_gradient, PARAMS must have a NUMITERS field. Use optimparams to set automatically.');
else
	numIters = params.numIters;
end;

if (~isfield(params, 'eta0')),
	error('When used with stochastic_gradient, PARAMS must have an ETA0 field. Use optimparams to set automatically.');
else
	eta0 = params.eta0;
end;

if (~isfield(params, 'stepUpdate')),
	error('When used with stochastic_gradient, PARAMS must have a STEPUPDATE field. Use optimparams to set automatically.');
else
	stepUpdate = params.stepUpdate;
end;

if (~isfield(params, 'store')),
	error('When used with stochastic_gradient, PARAMS must have a STORE field. Use optimparams to set automatically.');
else
	store = params.store;
end;
...
if (~isfield(params, 'verbose')),
	error('When used with stochastic_gradient, PARAMS must have a VERBOSE field. Use optimparams to set automatically.');
else
